package com.fengjunzi.algorithms;//给你一个变量对数组 equations 和一个实数值数组 values 作为已知条件，其中 equations[i] = [Ai, Bi] 和
...
// 
//
// Related Topics 深度优先搜索 广度优先搜索 并查集 图 数组 字符串 最短路 👍 1200 👎 0
...
class CalcEquation {
    public double[] calcEquation(List<List<String>> equations, double[] values, List<List<String>> queries) {
        ...
        for(int i =0;i<queries.size();i++){
            ...
            res[i] = dfs(equationMap, k1,k2, new HashSet<>());
        }
        return res;
    }

    public double dfs(Map<String, Map<String, Double>> equationMap, String k1, String k2, Set<String> path){
        ...
        for(Map.Entry<String, Double> entry : k1Map.entrySet()){
            ...
            double dfs = dfs(equationMap, key, k2, path);
            if(dfs != -1.0){
                return value * dfs;
            }
        }
